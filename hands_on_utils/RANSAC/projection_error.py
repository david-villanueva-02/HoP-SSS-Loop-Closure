...
def projection_error(H12, CL1uv, CL2uv):
    ...
    # Project coordinates of first image onto second one
    errors = []
    ...
    for i in range(len(CL1uv)):
        ...
        p1 = np.array([[p1[0]], [p1[1]], [1]]) # Homogenous
        projected_point = H12 @ p1

        if abs(projected_point[2,0]) < 1e-12:
            errors.append(np.inf)
            continue

        # Calculate euclidean error
        du = p2[0] - projected_point[0,0]/projected_point[2,0]
        dv = p2[1] - projected_point[1,0]/projected_point[2,0]
