...
# Make imports work regardless of the caller's cwd. `src/` (this file's own
# directory) goes on sys.path so the sibling modules (`RANSAC`, `xtf_utils`)
# resolve. MINIMA isn't pip-installable, so the repo root goes on sys.path so
# `from MINIMA.load_model ...` resolves, and MINIMA/ itself so the
# `from third_party.LightGlue ...` line inside load_model resolves at runtime.
_SRC_DIR = Path(__file__).resolve().parent
_REPO_ROOT = _SRC_DIR.parent
_MINIMA_DIR = _REPO_ROOT / "MINIMA"
for _p in (_SRC_DIR, _REPO_ROOT, _MINIMA_DIR):
...
class RegistrationResult:
    ...
    matcher_time_s: Optional[float] = None
...
def run_parametrized_minima_registration(
    ...
    _t_match0 = time.perf_counter()
    mkpts0, mkpts1, mconf = matcher.match_images(img0, img1)
    matcher_time_s = time.perf_counter() - _t_match0
