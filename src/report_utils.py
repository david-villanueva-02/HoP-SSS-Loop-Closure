...
Typical use from a notebook:
    ...
    def run_registration(matcher, spec):
        rk = spec.ransac_kwargs or {}
        return run_parametrized_minima_registration(
            ...
            ransac_reproj_threshold_pixel=rk.get("ransac_reproj_threshold_pixel", RANSAC_REPROJ_THRESHOLD_PIXEL),
            ransac_reproj_threshold_utm=rk.get("ransac_reproj_threshold_utm", RANSAC_REPROJ_THRESHOLD_UTM),
            ransac_max_iters=rk.get("ransac_max_iters", RANSAC_MAX_ITERS),
            ransac_confidence=rk.get("ransac_confidence", RANSAC_CONFIDENCE),
            ransac_outlier_percent=rk.get("ransac_outlier_percent", RANSAC_OUTLIER_PERCENT),
            flip_left_image=FLIP_LEFT_INPUT_IMAGE,
            apply_utm_distance_filter=rk.get("apply_utm_distance_filter", APPLY_UTM_DISTANCE_FILTER),
            utm_distance_threshold=rk.get("utm_distance_threshold", UTM_DISTANCE_THRESHOLD_METERS),
        )
...
class SweepSpec:
    ...
    # Optional per-spec RANSAC overrides. Recognised keys:
    #   ransac_reproj_threshold_pixel, ransac_reproj_threshold_utm,
    #   ransac_max_iters, ransac_confidence, ransac_outlier_percent,
    #   apply_utm_distance_filter, utm_distance_threshold
    ransac_kwargs: dict = field(default_factory=dict)
...
def default_sweep_specs_expanded() -> list[SweepSpec]:
    """
    Expanded grid used by the report pipeline: 4 backends with multiple
    matcher-parameter variants plus a small RANSAC sweep on the best matcher
    config per method.
    """
    specs: list[SweepSpec] = []
    ...
    for image_source in ("raw", "rho_gray"):
        ...
        for ft in (0.1, 0.2):
            specs.append(SweepSpec(
                name=f"lgsift_ft{ft}_{src_short}",
                backend="lightglue_sift",
                matcher_kwargs={"filter_threshold": ft, "max_num_keypoints": 4096},
                image_source=image_source,
                method_label="LightGlue+SIFT",
                variant_label=f"ft={ft}",
            ))

        specs.append(SweepSpec(
            name=f"minima_{src_short}",
            backend="minima_sp_lg",
            matcher_kwargs={},
            image_source=image_source,
            method_label="MINIMA(sp_lg)",
    ...
    # RANSAC variants on rho_gray only, to keep total runtime bounded.
    for utm_thr in (2.0, 5.0):
        for op in (0.5, 0.7):
            for backend, mlabel, mkw, prefix in (
                ("sift", "SIFT", {"ratio_test": 0.75, "nfeatures": 8000}, "sift"),
                ("lightglue_superpoint", "LightGlue+SuperPoint",
                    {"filter_threshold": 0.1, "max_num_keypoints": 4096}, "lgsp"),
                ("minima_sp_lg", "MINIMA(sp_lg)", {}, "minima"),
            ):
                specs.append(SweepSpec(
                    name=f"{prefix}_ransac_t{utm_thr}_op{op}_rho",
                    backend=backend,
                    matcher_kwargs=mkw,
                    image_source="rho_gray",
                    method_label=mlabel,
                    variant_label=f"ransac t={utm_thr}, op={op}",
                    ransac_kwargs={
                        "ransac_reproj_threshold_utm": utm_thr,
                        "ransac_outlier_percent": op,
                    },
                ))
...
def run_sweep(
    ...
    run_registration: Callable[[Any, SweepSpec], Any],
    ...
    ``run_registration`` receives ``(matcher, spec)``; the spec carries
    ``image_source`` and an optional ``ransac_kwargs`` dict to override the
    notebook globals on a per-spec basis.
    """
    ...
    for i, spec in enumerate(specs):
        ...
        try:
            ...
            result = run_registration(matcher, spec)
        ...
        for s in summaries:
            ...
            for r in s.rows:
                if r.get("method") != method:
                    continue
                if source_filter is not None and r.get("image_source") != source_filter:
                    continue
                vals.append(r.get(metric_key))
                variants_seen.add(str(r.get("variant", "")))
            ys.append(_aggregate(vals, agg))
        variants_str = ", ".join(sorted(v for v in variants_seen if v))
        label = (f"{method}  ({agg} over {len(variants_seen)} variant"
                 f"{'s' if len(variants_seen) != 1 else ''}: {variants_str})")
        ax.plot(x, ys, "-o", color=color, label=label, linewidth=2)
...
def _utm_inlier_count(result) -> int:
    """
    Number of UTM-RANSAC inliers. Returns 0 when UTM RANSAC failed
    (``H_utm is None`` / ``ransac_inliers_utm is None``), which is the correct
    funnel terminator: pixel-RANSAC inliers from the post-mask set must not be
    reported here, since they would violate ``n_inliers <= n_after_utm_dist``.
    """
    if result is None or getattr(result, "ransac_inliers_utm", None) is None:
        return 0
    return int(np.sum(result.ransac_inliers_utm))
...
def _safe_filename(text: str) -> str:
    """Turn a method label into a filename-safe stem."""
    out = "".join(c if c.isalnum() else "_" for c in text)
    while "__" in out:
        out = out.replace("__", "_")
    return out.strip("_").lower() or "method"


def plot_filtering_funnel(runs: list[RunRecord], out_dir: Path,
                          group_by_image_source: bool = True) -> list[Path]:
    """
    Match-count evolution through the pipeline, **one figure per model** so
    parameter variants don't crowd each other.

    Within each figure, every ``variant_label`` gets a distinct colour from
    ``tab10``/``tab20`` so variants are visually separable. The y-axis is
    non-negative: linear when counts fit in a single decade, log (lower bound
    = 1) otherwise.

    Saves one PNG per ``(method, image_source)`` as
    ``out_dir/funnel_<method>_<image_source>.png``.
    """
    ...
    for src in sources:
        runs_src = [r for r in runs
                    ...
                    and (src is None or r.spec.image_source == src)]
        if not runs_src:
            continue
        ...
        for method in methods:
            ...
            colors = _color_cycle(len(method_runs))

            fig, ax = plt.subplots(figsize=(10, 6))
            x = np.arange(len(_STAGES))

            max_count = 0
            for color, r in zip(colors, method_runs):
                counts = _funnel_counts(r.result)
                max_count = max(max_count, max(counts))
                variant = r.spec.variant_label or "default"
                ax.plot(x, counts, "-o", color=color,
                        label=variant, linewidth=1.8, markersize=5)

            ax.set_xticks(x)
            ax.set_xticklabels(_STAGE_LABELS)
            ax.set_ylabel("Number of matches")
            title_src = f"image_source = {src}" if src else "all image sources"
            ax.set_title(f"Match-count evolution: {method} ({title_src})")

            if max_count >= 1000:
                ax.set_yscale("log")
                ax.set_ylim(bottom=1, top=max(max_count * 1.2, 10))
            else:
                ax.set_yscale("linear")
                ax.set_ylim(bottom=0, top=max(max_count * 1.1, 1))

            ax.grid(True, which="both", alpha=0.3)
            ax.legend(loc="best", fontsize=9, title="Variant")
            fig.tight_layout()

            method_stem = _safe_filename(method)
            src_suffix = f"_{src}" if src else ""
            path = out_dir / f"funnel_{method_stem}{src_suffix}.png"
...
# ---------------------------------------------------------------------------
# Plot: matches at every pipeline stage
# ---------------------------------------------------------------------------

_MATCH_STAGES = ("raw", "after_mask", "after_utm", "ransac")
_MATCH_STAGE_LABELS = {
    "raw": "Raw matches",
    "after_mask": "After mask filter",
    "after_utm": "After UTM distance filter",
    "ransac": "RANSAC inliers",
}


def _draw_matches_panel(
    img0: np.ndarray,
    img1: np.ndarray,
    mkpts0: np.ndarray,
    mkpts1: np.ndarray,
    ...
    """OpenCV match visualisation returning an RGB image."""
    img0_u8 = _to_gray_u8(img0)
    img1_u8 = _to_gray_u8(img1)

    if len(mkpts0) > max_draw:
        rng = np.random.default_rng(0)
        idx = rng.choice(len(mkpts0), size=max_draw, replace=False)
        mkpts0 = mkpts0[idx]
        mkpts1 = mkpts1[idx]
...
def _stage_match_arrays(result, stage: str) -> tuple[np.ndarray, np.ndarray]:
    """
    Return the (mkpts0, mkpts1) match pair at the given pipeline stage.

    Stages: raw, after_mask, after_utm, ransac. Returns empty arrays when the
    data needed for that stage is unavailable.
    """
    ...
    if stage == "raw":
        return (np.asarray(result.mkpts0, dtype=np.float32),
                np.asarray(result.mkpts1, dtype=np.float32))

    if stage == "after_mask":
        return (np.asarray(result.mkpts0_kept, dtype=np.float32),
                np.asarray(result.mkpts1_kept, dtype=np.float32))

    kept0 = np.asarray(getattr(result, "mkpts0_kept", None), dtype=np.float32) \
        if getattr(result, "mkpts0_kept", None) is not None else None
    kept1 = np.asarray(getattr(result, "mkpts1_kept", None), dtype=np.float32) \
        if getattr(result, "mkpts1_kept", None) is not None else None

    if stage == "after_utm":
        keep = getattr(result, "utm_distance_keep_mask", None)
        if kept0 is None or kept1 is None or keep is None or len(kept0) != len(keep):
            return empty
        keep = np.asarray(keep, dtype=bool)
        return kept0[keep], kept1[keep]

    if stage == "ransac":
        inliers = getattr(result, "ransac_inliers_utm", None)
        if kept0 is None or kept1 is None or inliers is None or len(kept0) != len(inliers):
            return empty
        inliers = np.asarray(inliers, dtype=bool)
        return kept0[inliers], kept1[inliers]

    raise ValueError(f"Unknown stage: {stage!r}")


def plot_matches_per_run_stages(run: RunRecord, out_dir: Path,
                                max_draw: int = 300) -> dict[str, Path]:
    """
    Per-run match-overlay PNGs at every pipeline stage.

    Output under ``<out_dir>/runs/<spec_name>/``:
        matches_raw.png, matches_after_mask.png, matches_after_utm.png,
        matches_ransac.png and the combined matches_stages.png (4 stacked panels).
    """
    saved: dict[str, Path] = {}
    ...
    img0 = run.result.img0
    img1 = run.result.img1

    panels: dict[str, np.ndarray] = {}
    counts: dict[str, int] = {}

    for stage in _MATCH_STAGES:
        p0, p1 = _stage_match_arrays(run.result, stage)
        counts[stage] = len(p0)
        if len(p0) == 0:
            panel = _draw_matches_panel(img0, img1, p0, p1, max_draw=max_draw)
        else:
            panel = _draw_matches_panel(img0, img1, p0, p1, max_draw=max_draw)
        panels[stage] = panel
        ...
        ax.imshow(panel)
        ax.set_title(f"{_MATCH_STAGE_LABELS[stage]}  n={counts[stage]}  ({run.spec.name})")
        ax.axis("off")
        fig.tight_layout()
        path = run_dir / f"matches_{stage}.png"
        fig.savefig(path, dpi=150)
        plt.close(fig)
        saved[stage] = path

    fig, axes = plt.subplots(len(_MATCH_STAGES), 1, figsize=(14, 4.5 * len(_MATCH_STAGES)))
    for ax, stage in zip(axes, _MATCH_STAGES):
        ax.imshow(panels[stage])
        ax.set_title(f"{_MATCH_STAGE_LABELS[stage]}  n={counts[stage]}")
        ax.axis("off")
    fig.suptitle(
        f"{run.spec.name}  ({run.spec.method_label} | {run.spec.variant_label} | {run.spec.image_source})"
    )
    fig.tight_layout()
    path = run_dir / "matches_stages.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    saved["stages"] = path
...
def plot_combined_matches_per_stage(runs: list[RunRecord], out_dir: Path,
                                    max_draw: int = 200) -> dict[str, Path]:
    """
    For each pipeline stage produce one bundled PNG that stacks the match
    visualisation of every (method, variant, image_source) run as panels in a
    single figure. Outputs under ``out_dir/``:

        bundled_matches_raw.png, bundled_matches_after_mask.png,
        bundled_matches_after_utm.png, bundled_matches_ransac.png

    Empty / failed runs render as a "no matches" placeholder panel so the
    grid stays aligned across stages.
    """
    out_dir = Path(out_dir)
    saved: dict[str, Path] = {}
    ...
    for stage in _MATCH_STAGES:
        ...
        for ax, run in zip(axes[:, 0], runs_ok):
            p0, p1 = _stage_match_arrays(run.result, stage)
            count = len(p0)
            title = (
                f"{run.spec.method_label} | {run.spec.variant_label} "
                f"| {run.spec.image_source} | n={count}"
            )
            if count == 0:
                ...
                panel = _draw_matches_panel(
                    run.result.img0, run.result.img1, p0, p1, max_draw=max_draw,
                )
                ax.imshow(panel)
                ax.set_title(title, fontsize=10)
                ax.axis("off")

        fig.suptitle(f"Bundled matches across methods: {_MATCH_STAGE_LABELS[stage]}",
                     fontsize=12)
        fig.tight_layout()
        path = out_dir / f"bundled_matches_{stage}.png"
...
def plot_inference_time_histogram(runs: list[RunRecord], out_dir: Path,
                                  fps_lines: tuple[float, ...] = (30.0, 10.0)
                                  ) -> Optional[Path]:
    """
    Overlaid histograms of the matcher inference time (matcher.match_images
    only, not the whole pipeline) grouped by ``method_label``.

    Vertical reference lines at the given FPS thresholds make real-time
    viability easy to read.
    """
    samples_by_method: dict[str, list[float]] = {}
    for run in runs:
        if run.result is None:
            continue
        t = getattr(run.result, "matcher_time_s", None)
        if t is None or not np.isfinite(t):
            continue
        samples_by_method.setdefault(run.spec.method_label, []).append(float(t))
    ...
    all_samples = np.concatenate([np.asarray(v) for v in samples_by_method.values()])
    upper = float(np.percentile(all_samples, 99)) if all_samples.size else 1.0
    upper = max(upper, max((1.0 / f for f in fps_lines), default=upper))
    bins = np.linspace(0.0, upper, 40)

    fig, ax = plt.subplots(figsize=(11, 6))
    for color, method in zip(colors, methods):
        arr = np.asarray(samples_by_method[method], dtype=float)
        median = float(np.median(arr))
        p95 = float(np.percentile(arr, 95))
        ax.hist(arr, bins=bins, color=color, alpha=0.45, edgecolor="none",
                label=f"{method}  median={median:.3f}s  p95={p95:.3f}s  n={len(arr)}")

    for fps in fps_lines:
        ax.axvline(1.0 / fps, color="black", linestyle="--", linewidth=0.8, alpha=0.5)
        ax.text(1.0 / fps, ax.get_ylim()[1] * 0.95, f"{fps:g} FPS",
                rotation=90, va="top", ha="right", fontsize=8, alpha=0.6)

    ax.set_xlabel("Matcher inference time (s)")
    ax.set_ylabel("Run count")
    ax.set_xlim(left=0.0)
    ax.set_title("Inference-time distribution per method")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best", fontsize=9)
    fig.tight_layout()

    path = Path(out_dir) / "inference_times.png"
    fig.savefig(path, dpi=200)
    plt.show()
    plt.close(fig)
    return path
...
_SUMMARY_FIELDS = [
    ...
    "n_raw", "n_after_mask", "n_after_utm_dist", "n_inliers", "n_pixel_inliers",
    "residual_mean_m", "residual_median_m", "residual_p95_m", "residual_rmse_m",
    "tx_utm_m", "ty_utm_m", "theta_utm_deg",
    "matcher_time_s", "runtime_s", "h_utm_ok", "error",
]
...
def _row_for(run: RunRecord) -> dict:
    r = run.result
    n_pixel = 0
    if r is not None and getattr(r, "ransac_inliers_pixel", None) is not None:
        n_pixel = int(np.sum(r.ransac_inliers_pixel))
    ...
    row = {
        ...
        # n_inliers is UTM-RANSAC inliers only (the actual final pipeline stage).
        # Pixel-RANSAC inliers are kept in a separate column.
        "n_inliers": _utm_inlier_count(r),
        "n_pixel_inliers": n_pixel,
        ...
        "matcher_time_s": (getattr(r, "matcher_time_s", None) if r is not None else None),
...
def generate_full_report(
    ...
    out["inference_times"] = plot_inference_time_histogram(runs, out_dir)
    out["matches_per_run"] = [plot_matches_per_run_stages(r, out_dir) for r in runs]
    out["bundled_matches"] = plot_combined_matches_per_stage(runs, out_dir)
    return out
