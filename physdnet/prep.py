...
def prepare_data_range(input_dir: str, xtf_file: str, output_dir: str, segment_size: int, overlap_size: int, upper_limit: int, side: str, data_lower_limit: int, data_upper_limit: int):
    '''Prepares the dataset by reading XTF files, processing them, and saving the results in a structured format.
    It works for side = left, right, or both.'''
    ...
    print(f'Processing file: {xtf_file}')
    try:
        file_header, packets = pyxtf.xtf_read(file_path)
        #print("packets: ", packets_complete)
        #packets = packets_complete[data_lower_limit:data_upper_limit]
        #print(packets)
        sonar_packets: list[pyxtf.XTFPingHeader] = packets[pyxtf.XTFHeaderType.sonar]
        ...
        if left_channel_data.size > 0 and right_channel_data.size > 0:
            ...
            combined_image = np.hstack((left_image_normalized, right_image_normalized))
            cv2.imwrite(os.path.join(output_dir, f"{xtf_file}_combined.png"), combined_image)
            ...
            # 如果最后剩余部分没有被覆盖，可以补一个最后窗口
            if len(start_positions) == 0:
                print(f"Warning: File {xtf_file} 行数不足一个 segment,跳过")
            ...
            for i, start in enumerate(start_positions):
                ...
                if side == "left" or side == "both":
                    ...
                    cv2.imwrite(
                        os.path.join(output_dir, "images", f"{xtf_file}_left_{i:03d}.png"),
                        flipped_image_left
                    )
                    cv2.imwrite(
                        os.path.join(output_dir, "shadow", f"{xtf_file}_left_{i:03d}.png"),
                        left_mask
                    )
                    np.save(
                        os.path.join(output_dir, "altitude", f"{xtf_file}_left_{i:03d}.npy"),
                        altitude_left_segment
                    )
                    np.save(
                        os.path.join(output_dir, "range", f"{xtf_file}_left_{i:03d}.npy"),
                        range_left_segment
                    )
                ...
                if side == "right" or side == "both":
                    ...
                    cv2.imwrite(
                        os.path.join(output_dir, "images", f"{xtf_file}_right_{i:03d}.png"),
                        image_right_data
                    )
                    cv2.imwrite(
                        os.path.join(output_dir, "shadow", f"{xtf_file}_right_{i:03d}.png"),
                        right_mask
                    )
                    np.save(
                        os.path.join(output_dir, "altitude", f"{xtf_file}_right_{i:03d}.npy"),
                        altitude_right_segment
                    )
                    np.save(
                        os.path.join(output_dir, "range", f"{xtf_file}_right_{i:03d}.npy"),
                        range_right_segment
                    )

                cv2.imwrite(
                    os.path.join(output_dir, f"{xtf_file}_mask_left.png"),
                    mask_left_inv
                )
                cv2.imwrite(
                    os.path.join(output_dir, f"{xtf_file}_mask_right.png"),
                    mask_right_inv
                )

                print(f"Saved in {os.path.join(output_dir, f"{xtf_file}_mask_right.png")}")

            print(f"{xtf_file}: total_rows={total_rows}, segments={len(start_positions)}, stride={stride}")

        else:
            print(f"Warning: File {xtf_file} has no valid channel data")

    except Exception as e:
        print(f"Error reading or processing {xtf_file}: {e}")

    return sonar_packets
...
if __name__ == "__main__":
    ...
    prepare_data(input_dir, output_dir, segment_size, overlap_size, upper_limit, side="both")